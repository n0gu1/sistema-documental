...
function RolesView({ globalQuery = '' }) {
  ...
  const selectedRoleActive = roles.find((role) => role.id === selectedRoleId)?.activo

  async function loadRoles() {
    try {
      const result = await apiRequest('/api/admin/roles/?limit=100&include_inactive=true')
  ...
  useEffect(() => {
    ...
    if (selectedRoleActive === false) { setGrants(new Set()); return }
  ...
  }, [selectedRoleId, selectedRoleActive])
  ...
  return <div className="roles-view">
    <header className="roles-heading"><div><h1>Roles y permisos</h1><p>Administre perfiles y autorizaciones reales de la organización.</p></div><div className="roles-heading__actions"><PermissionButton permission="roles.gestionar" type="button" onClick={() => setShowCreate(true)}><RoleIcon name="plus" /> Nuevo rol</PermissionButton><PermissionButton permission="roles.gestionar" className="is-primary" type="button" onClick={saveChanges}><RoleIcon name="save" /> Guardar cambios</PermissionButton></div></header>
    {error && <p className="roles-error" role="alert">{error}</p>}{notice && <p className="roles-notice" role="status">{notice}</p>}
    {selectedRole && !editingRole && <PermissionButton permission="roles.gestionar" className="roles-edit-selected" type="button" onClick={() => setEditingRole(selectedRole)}><RoleIcon name="edit" size={16} /> Editar rol seleccionado</PermissionButton>}
    {editingRole && <RoleEditForm key={editingRole.id} role={editingRole} onCancel={() => setEditingRole(null)} onSaved={() => { setEditingRole(null); setNotice('Rol actualizado correctamente.'); loadRoles() }} />}
    {showCreate && <PermissionForm permission="roles.gestionar" className="roles-create-panel" onSubmit={createRole}><h2>Crear rol</h2><label>Código<input required pattern="[A-Za-z0-9_-]+" value={form.code} onChange={(event) => setForm({ ...form, code: event.target.value.toUpperCase() })} /></label><label>Nombre<input required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Descripción<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><button type="submit">Crear rol</button><button type="button" onClick={() => setShowCreate(false)}>Cancelar</button></PermissionForm>}
     <section className="roles-metrics" aria-label="Indicadores de roles y permisos"><article><span className="roles-metric-icon roles-metric-icon--blue"><RoleIcon name="roles" size={27} /></span><div><p>Total de roles</p><strong>{roles.length}</strong><small>activos e inactivos</small></div></article><article><span className="roles-metric-icon roles-metric-icon--green"><RoleIcon name="key" size={27} /></span><div><p>Permisos definidos</p><strong>{activePermissions.length}</strong><small>{permissions.length} en catálogo</small></div></article><article><span className="roles-metric-icon roles-metric-icon--orange"><RoleIcon name="shield" size={27} /></span><div><p>Rol seleccionado</p><strong>{selectedRole ? selectedRole.nombre || selectedRole.name : '—'}</strong><small>configuración actual</small></div></article><article><span className="roles-metric-icon roles-metric-icon--violet"><RoleIcon name="check" size={27} /></span><div><p>Permisos otorgados</p><strong>{grants.size}</strong><small>en el rol seleccionado</small></div></article></section>
