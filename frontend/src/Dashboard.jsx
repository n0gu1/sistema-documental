import { usePermissions, PermissionGate, PermissionButton } from './Permissions'
import { managementViews } from './permissionPolicy'
  ...
  return <main className="dashboard-shell">
    <aside className={`dashboard-sidebar${sidebarOpen ? ' dashboard-sidebar--open' : ''}`}>
      ...
      <nav className="dashboard-nav" aria-label="Navegación principal">
        <p>Administración</p>
        {navigation.filter(([view]) => can(managementViews[view])).map(([icon, label]) => <button className={activeView === icon ? 'dashboard-nav__item dashboard-nav__item--active' : 'dashboard-nav__item'} type="button" key={label} onClick={() => navigate(icon)}><Icon name={icon} size={21} /><span>{label}</span></button>)}
      </nav>
    ...
    <section className="dashboard-workspace">
      <header className="dashboard-topbar"><button className="dashboard-menu" type="button" aria-label="Abrir menú" onClick={() => setSidebarOpen(true)}><Icon name="menu" size={24} /></button><label className="dashboard-search"><Icon name="search" size={19} /><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar documentos, versiones, usuarios..." aria-label="Buscar" /><kbd>⌘ K</kbd></label><div className="dashboard-topbar__actions"><NotificationBell className="dashboard-notification" iconSize={22} /><div className="dashboard-profile"><button className="dashboard-profile__trigger" type="button" aria-expanded={profileOpen} onClick={() => setProfileOpen((open) => !open)}><span className="dashboard-avatar">{initials}</span><span className="dashboard-profile__text"><strong>{user.full_name}</strong><small>{role}</small></span><Icon name="chevron" size={17} /></button>{profileOpen && <div className="dashboard-profile__menu"><span>{user.email}</span><button type="button" onClick={onLogout} disabled={logoutPending}><Icon name="logout" size={18} /> {logoutPending ? 'Cerrando...' : 'Cerrar sesión'}</button></div>}</div></div></header>
      <div className="dashboard-content"><PermissionGate permission={managementViews[activeView]} fallback={<p>No tiene permiso para esta función.</p>}>
        ...
        {activeView === 'document' ? documentMode === 'edit' && selectedDocumentId ?
          ...
          <div className="dashboard-heading"><div><p>Vista general</p><h1>Panel de administración</h1><span>Supervise la actividad documental y el estado de la organización.</span></div><div className="dashboard-date"><Icon name="calendar" size={18} /><span>{today}</span></div></div>
           ...
           {data.security_alerts.length > 0 && <section className="dashboard-panel dashboard-security-alerts" aria-labelledby="dashboard-security-alerts-title"><div className="dashboard-panel__heading"><div><h2 id="dashboard-security-alerts-title"><Icon name="bell" size={17} /> Alertas de seguridad</h2><p>{data.security_alert_count} grupos requieren revisión</p></div><PermissionButton permission={managementViews['clipboard']} type="button" onClick={() => navigate('clipboard')}>Ver bitácora <Icon name="arrow" size={15} /></PermissionButton></div><div className="dashboard-security-alerts__list">{data.security_alerts.map((alert) => <article key={alert.id}><span><Icon name="shield" size={17} /></span><div><strong>{alert.title}</strong><p>{alert.message}</p><small>{alert.source}</small></div><b>{alert.severity === 'critico' ? 'Crítico' : 'Alto'}</b></article>)}</div></section>}
           <div className="dashboard-primary-grid"><section className="dashboard-panel dashboard-documents"><div className="dashboard-panel__heading"><div><h2>Documentos recientes</h2><p>Últimos documentos modificados en la organización</p></div><PermissionButton permission={managementViews['document']} type="button" onClick={() => navigate('document')}>Ver todos <Icon name="arrow" size={16} /></PermissionButton></div><div className="dashboard-table-wrap"><table><thead><tr><th>Documento</th><th>Tipo</th><th>Responsable</th><th>Estado</th><th>Versión</th><th>Actualización</th></tr></thead><tbody>{filteredDocuments.map((document, index) => <tr key={document.id}><td><div className="dashboard-document"><span className={`dashboard-document__icon dashboard-document__icon--${index % 4}`}><Icon name="document" size={18} /></span><div><strong>{document.title}</strong><small>{document.code}</small></div></div></td><td><span className="dashboard-type">{document.type}</span></td><td>{document.responsible}</td><td><span className="dashboard-status">{document.status}</span></td><td>{document.version || '—'}</td><td>{formatDate(document.updated_at)}</td></tr>)}</tbody></table>{!loading && !filteredDocuments.length && <p className="dashboard-empty">No hay documentos para mostrar.</p>}</div><div className="dashboard-table__footer"><span>{loading ? 'Cargando documentos...' : `Mostrando ${filteredDocuments.length} de ${data.metrics.total_documents} documentos`}</span><PermissionButton permission={managementViews['document']} type="button" onClick={() => navigate('document')}>Abrir repositorio</PermissionButton></div></section>
            <aside className="dashboard-side-column"><section className="dashboard-panel dashboard-actions"><div className="dashboard-panel__heading"><div><h2>Acciones rápidas</h2><p>Operaciones frecuentes</p></div></div><div className="dashboard-actions__grid"><PermissionButton permission={'documentos.crear'} type="button" onClick={() => navigate('document')}><span className="dashboard-tone--blue"><Icon name="plus" size={21} /></span><strong>Nuevo documento</strong><small>Abrir repositorio</small></PermissionButton><PermissionButton permission={managementViews['users']} type="button" onClick={() => navigate('users')}><span className="dashboard-tone--violet"><Icon name="users" size={21} /></span><strong>Administrar usuarios</strong><small>Gestionar accesos</small></PermissionButton><PermissionButton permission={managementViews['shield']} type="button" onClick={() => navigate('shield')}><span className="dashboard-tone--green"><Icon name="shield" size={21} /></span><strong>Roles y permisos</strong><small>Revisar autorizaciones</small></PermissionButton><PermissionButton permission={managementViews['clipboard']} type="button" onClick={() => navigate('clipboard')}><span className="dashboard-tone--orange"><Icon name="clipboard" size={21} /></span><strong>Ver bitácora</strong><small>Auditar actividad</small></PermissionButton></div></section><section className="dashboard-panel dashboard-review"><div className="dashboard-panel__heading"><div><h2>Cola de revisión</h2><p>Solicitudes pendientes</p></div><span>{data.metrics.pending_reviews}</span></div><div className="dashboard-review__progress"><span style={{ width: `${data.metrics.pending_reviews ? Math.max(8, Math.min(100, (data.metrics.overdue_reviews / data.metrics.pending_reviews) * 100)) : 0}%` }} /></div><strong>{data.metrics.overdue_reviews} revisiones vencidas</strong><p>{data.pending_queue.length ? `Próxima: ${data.pending_queue[0].code} · ${data.pending_queue[0].reviewer}` : 'No hay revisiones pendientes.'}</p><PermissionButton permission={managementViews['document']} type="button" onClick={() => navigate('document')}>Abrir documentos <Icon name="arrow" size={16} /></PermissionButton></section></aside></div>
          <div className="dashboard-secondary-grid"><section className="dashboard-panel dashboard-chart"><div className="dashboard-panel__heading"><div><h2>Estados documentales</h2><p>Distribución actual del repositorio</p></div><span><i /> Documentos</span></div><div className="dashboard-chart__body dashboard-status-chart">{Object.entries(data.statuses).map(([status, count]) => <div className="dashboard-status-row" key={status}><span>{status}</span><i><b style={{ width: `${(count / statusTotal) * 100}%` }} /></i><strong>{count}</strong></div>)}</div></section><section className="dashboard-panel dashboard-activity"><div className="dashboard-panel__heading"><div><h2>Actividad reciente</h2><p>Movimientos registrados en la bitácora</p></div><PermissionButton permission={managementViews['clipboard']} type="button" onClick={() => navigate('clipboard')}>Ver bitácora</PermissionButton></div><div className="dashboard-activity__list">{filteredActivity.map((item) => <article key={item.id}><span className={item.successful ? 'dashboard-tone--green' : 'dashboard-tone--orange'}><Icon name={item.successful ? 'check' : 'clock'} size={18} /></span><div><strong>{item.action}</strong><p>{item.user} · {item.detail}</p></div><time>{formatDate(item.at)}</time></article>)}{!loading && !filteredActivity.length && <p className="dashboard-empty">No hay actividad registrada.</p>}</div></section></div>
        </>}
