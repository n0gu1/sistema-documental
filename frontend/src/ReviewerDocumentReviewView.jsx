...
function ReviewerDocumentReviewView({ reviewId, onAction, onNavigate }) {
  ...
  return <div className="reviewer-document-review">
    ...
    {rejectionOpen && <PermissionForm permission="revisiones.rechazar" className="reviewer-rejection-form" onSubmit={rejectReview}><div><h2>Rechazo definitivo</h2><p>Esta acción marcará la versión como rechazada y cerrará las revisiones pendientes.</p></div><textarea value={rejectionComment} onChange={(event) => setRejectionComment(event.target.value)} placeholder="Explique el motivo del rechazo definitivo..." maxLength={2000} autoFocus /><footer><button type="button" onClick={() => { setRejectionOpen(false); setRejectionComment('') }}>Cancelar</button><button className="is-reject" type="submit">Confirmar rechazo</button></footer></PermissionForm>}
    <footer className="reviewer-review-actions"><button type="button" onClick={() => setActiveTab('Observaciones')}><ReviewIcon name="comment" size={18} />Guardar observación</button><button type="button" onClick={() => onNavigate?.('compare')}><ReviewIcon name="layers" size={18} />Comparar versiones</button><PermissionButton permission="revisiones.rechazar" type="button" className="is-return" disabled={!canDecide} onClick={() => decide('return')}><ReviewIcon name="close" size={18} />Devolver con observaciones</PermissionButton><PermissionButton permission="revisiones.rechazar" type="button" className="is-reject" disabled={!canDecide} onClick={() => { setError(''); setRejectionOpen(true) }}><ReviewIcon name="close" size={18} />Rechazar definitivamente</PermissionButton><PermissionButton permission="revisiones.aprobar" type="button" className="is-approve" disabled={!canDecide} onClick={() => decide('approve')}><ReviewIcon name="check" size={18} />Aprobar documento</PermissionButton></footer>
  </div>
}
