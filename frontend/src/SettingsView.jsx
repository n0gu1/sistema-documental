...
function SettingsView({ globalQuery }) {
  ...
  function renderIntegrations() {
    ...
    return <section className="settings-integrations"><p className="settings-help">Las integraciones externas se configuran por organización. Las credenciales sensibles no se muestran.</p><div className="settings-integration-grid">{items.map(([provider, name, description]) => { const item = integrations[provider] || { enabled: false, status: 'not_configured' }; return <article key={provider}><div><strong>{name}</strong><small>{description}</small></div><span className={item.status === 'configured' ? 'settings-connected' : 'settings-not-configured'}>{item.status === 'configured' ? 'Configurado' : 'No configurado'}</span><PermissionButton permission="usuarios.gestionar" type="button" onClick={() => testIntegration(provider)} disabled={saving}>Probar</PermissionButton></article> })}</div>{integrations.webhook && <Field label="URL del webhook"><input value={integrations.webhook.url || ''} onChange={(event) => setSettings((current) => ({ ...current, integrations: { ...current.integrations, webhook: { ...current.integrations.webhook, url: event.target.value } } }))} placeholder="https://integracion.ejemplo.com/webhook" /></Field>}</section>
  }
