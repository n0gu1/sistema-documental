...
function FilterSelect({ label, value, onChange, options, placeholder }) {
  return (
    <label className="editor-doc-filter">
      <span>{label}</span>
      <div>
        <select
          value={value}
        ...
        >
          ...
          {options.map((option) => (
            <option
              key={option.id || option.code || option.value || option.name}
        ...
        <DocumentsIcon name="chevron" size={15} />
      </div>
    </label>
  );
}
...
function EditorDocumentsView({ globalQuery, onAction, onEditDocument }) {
  ...
  useEffect(() => {
    ...
    apiRequest("/api/documents/catalogs/")
      .then((catalogData) => {
        if (active) setCatalogs(catalogData);
      })
      .catch((requestError) => {
  ...
  const pageDocuments = visibleDocuments;
  const statusSummary = statuses.map((item) => ({
    ...item,
    count: documents.filter((document) => document.status === item.name).length,
  }));
  ...
  return (
    <div className="editor-documents-view">
      <header className="editor-documents-heading">
        <div>
          <h1>Mis documentos</h1>
          <p>
            Crea, actualiza y envía tus documentos personales para revisión.
          </p>
        </div>
        <section className="editor-documents-summary">
          <div>
            <DocumentsIcon name="document" size={25} />
            <span>
              Total de documentos<strong>{total}</strong>
            </span>
          </div>
          <div>
            <DocumentsIcon name="clock" size={25} />
            <span>
              Última actualización
              <strong>{latestDocument?.updated || "Sin registros"}</strong>
            </span>
          </div>
        </section>
      </header>
      ...
      >
        {statusSummary.map((item) => (
          <article key={item.id}>
            <span className={`is-${statusTone(item.name)}`}>
              <DocumentsIcon
                name={
                  statusTone(item.name) === "review"
                    ? "clock"
                    : statusTone(item.name) === "approved"
                      ? "check"
                      : statusTone(item.name) === "returned"
                        ? "comment"
                        : "edit"
                }
                size={25}
            ...
            <div>
              ...
              <span>
                {documents.length
                  ? `${Math.round((item.count / documents.length) * 100)}% del total`
                  : "0% del total"}
              </span>
            </div>
          </article>
        ...
        {!loading && !statusSummary.length && (
          <p className="editor-doc-empty">No hay estados documentales.</p>
        )}
      </section>
      <section className="editor-documents-panel">
        <div className="editor-doc-filters">
          <label className="editor-doc-filter editor-doc-filter--search">
            <span>Buscar documento</span>
            <div>
              <DocumentsIcon name="search" size={17} />
              <input
                type="search"
                value={search}
                onChange={(event) => {
          ...
          <button
            className="editor-clear-filters"
            type="button"
            onClick={clearFilters}
          >
            <DocumentsIcon name="filter" size={16} /> Limpiar filtros
          </button>
        </div>
        <div className="editor-doc-table-wrap">
          <table>
            <thead>
              <tr>
                <th>Código</th>
            ...
            <tbody>
              {pageDocuments.map((document) => (
                <tr key={document.id}>
                  <td className="editor-doc-code">{document.code}</td>
                  <td>{document.title}</td>
                  <td>{document.type}</td>
                  <td>
                    <span
                      className={`editor-doc-status editor-doc-status--${statusTone(document.status)}`}
                    >
                      {document.status}
                    </span>
                  </td>
                  ...
                  <td>
                    ...
                    <small>
                      {formatDate(document.created_at, "Sin fecha")}
                    </small>
                  </td>
                  ...
                  <td>
                    <div className="editor-doc-actions">
                      ...
                      <button
                        ...
                        onClick={() =>
                          document.downloadUrl
                            ...
                            : onAction("No hay versión descargable.")
                        }
                      >
                      ...
                      <button
                        ...
                        onClick={() =>
                          onAction(`Más acciones de ${document.title}`)
                        }
                      >
                        <DocumentsIcon name="more" size={16} />
                      </button>
                    </div>
                  </td>
                </tr>
          ...
          {loading && (
            <div className="editor-doc-empty">Cargando documentos...</div>
          )}
          {!loading && !pageDocuments.length && (
            <div className="editor-doc-empty">
              <DocumentsIcon name="search" size={25} />
              <strong>Sin resultados</strong>
              <span>Prueba con otros términos o limpia los filtros.</span>
            </div>
          )}
        </div>
        <footer className="editor-doc-pagination">
          <span>
            Mostrando{" "}
            {total ? (currentPage - 1) * PAGE_SIZE + 1 : 0} a{" "}
            {Math.min(currentPage * PAGE_SIZE, total)} de {total} documentos
          </span>
          <div>
            <button
              type="button"
