...
function DocumentViewIcon({ name, size = 18 }) {
  const paths = {
    ...
    refresh: (
      <>
        <path d="M20 7v5h-5" />
        <path d="M18.5 16a8 8 0 1 1 1.2-8.5L20 12" />
      </>
    ),
    eye: (
      <>
    ...
    sort: (
      <>
        <path d="M8 5v14m0-14L5 8m3-3 3 3M16 19V5m0 14-3-3m3 3 3-3" />
      </>
    ),
    star: (
  ...
  return (
    <label className="documents-filter">
      ...
      <div>
        ...
        <DocumentViewIcon name="chevron" size={15} />
      </div>
    </label>
  );
}
...
function DocumentsView({ globalQuery, today, onOpenVersions }) {
  ...
  const [classification, setClassification] = useState("-updated_at");
  ...
  useEffect(() => {
    ...
    const query = buildDocumentQuery({
      ...
      from,
      until,
       ordering: classification === "Todas" ? "-updated_at" : classification,
       limit: PAGE_SIZE,
  ...
  const statusOptions = [
    "Todos",
    ...(catalogs.statuses || []).map((item) => item.name),
  ];
  const counts = statusOptions
    .slice(1)
    .map((item) => [
      item,
      documents.filter((document) => document.status === item).length,
    ]);
  ...
  async function openCreate(mode) {
    ...
    try {
      setCatalogs(await apiRequest("/api/documents/catalogs/"));
    } catch (requestError) {
    ...
    <div className="documents-view">
      <div className="documents-hero">
        <div className="documents-title">
          <div>
            ...
            <span>
              Administre, clasifique y controle los documentos institucionales
              de la organización.
            </span>
          </div>
          <div className="documents-date">
            <DocumentViewIcon name="calendar" size={18} />
            {today}
          </div>
          <div className="documents-toolbar">
            ...
            <button
              ...
              onClick={exportList}
            >
              <DocumentViewIcon name="download" size={19} /> Exportar listado
            </button>
          </div>
        </div>
        <section className="documents-summary" aria-label="Resumen documental">
          <div className="documents-summary__items">
            {counts.map(([label, count], index) => (
              <article key={label}>
                <span
                  className={`documents-summary__icon documents-summary__icon--${["blue", "orange", "green", "violet"][index % 4]}`}
                >
                  <DocumentViewIcon
                    name={
                      index === 0
          ...
          <footer>
            ...
            <span>
              ...
              <DocumentViewIcon name="refresh" size={17} />
            </span>
          </footer>
        </section>
      </div>
      <section className="documents-filters">
        <label className="documents-filter documents-filter--search">
          <span>Búsqueda</span>
          <div>
            <DocumentViewIcon name="search" size={17} />
            <input
              type="search"
              value={search}
              onChange={(event) => {
        ...
        <SelectFilter
          label="Responsable"
          value={owner}
          onChange={updateFilter(setOwner)}
          options={[
            "Todos",
            ...(catalogs.responsibles || []).map((item) => item.name),
          ]}
        />
        ...
        <SelectFilter
          ...
          onChange={updateFilter(setClassification)}
          options={DOCUMENT_ORDERING_OPTIONS}
        />
        <div className="documents-filter-actions">
          <button type="button" onClick={clearFilters}>
            <DocumentViewIcon name="filter" size={17} /> Limpiar filtros
          </button>
          <button
          ...
          >
            <DocumentViewIcon name="sliders" size={17} /> Filtros avanzados{" "}
            <DocumentViewIcon name="chevron" size={14} />
          </button>
        </div>
        {advancedOpen && (
          <p className="documents-advanced-message">
            Los filtros se aplican remotamente sobre el catálogo documental.
          </p>
        )}
      </section>
      {error && (
      ...
      <section className="documents-table-panel">
        <div className="documents-table-scroll">
                  ...
                  <tr key={document.id}>
                    ...
                    <td>
                      <span className="documents-name">
                        {document.title}
                        {document.favorite && (
                          <span className="documents-favorite">
                            <DocumentViewIcon name="star" size={13} />
                          </span>
                        )}
                      </span>
                    </td>
                    <td>
                      <span
                        className={`documents-type documents-type--${typeTones[document.type] || "blue"}`}
                      >
                        {document.type}
                      </span>
                    </td>
                    <td>{document.area}</td>
                    <td>
                      <span className="documents-owner">
                        <i className="documents-owner__avatar documents-owner__avatar--blue">
                          {initials}
                        </i>
                        {ownerName}
                      </span>
                    ...
                    <td>
                      <span className="documents-state">{document.status}</span>
                    </td>
                    <td>{document.version}</td>
                    <td>
                      <span className="documents-updated">
                    ...
                    <td>
                      <div className="documents-row-actions">
                        ...
                        <button
                          ...
                          aria-label={`Ver versiones de ${document.title}`}
                          onClick={() => onOpenVersions?.(document.id)}
                        >
                          <DocumentViewIcon name="copy" size={16} />
                        </button>
                        <button
                          type="button"
                        ...
                        >
                          <DocumentViewIcon name="share" size={16} />
                        </button>
                        <button
                          type="button"
                          aria-label={`Más acciones para ${document.title}`}
                          onClick={() => onOpenVersions?.(document.id)}
                        >
                          <DocumentViewIcon name="more" size={16} />
                        </button>
                      </div>
                    </td>
                  </tr>
          ...
          {loading && (
            <div className="documents-empty">Cargando documentos...</div>
          )}
          {!loading && !visibleDocuments.length && (
            <div className="documents-empty">
              <DocumentViewIcon name="search" size={23} />
              <strong>Sin resultados</strong>
              <span>
                Pruebe con otros términos o limpie los filtros seleccionados.
              </span>
            </div>
          )}
        </div>
          ...
          <form onSubmit={createDocument}>
            ...
            <label>
              ...
              <select
                ...
                value={form.type_id}
                onChange={(event) =>
                  setForm({ ...form, type_id: event.target.value })
                }
              >
            ...
            <footer>
              ...
              <button
                ...
                type="submit"
                disabled={
                  saving || !catalogs.areas.length || !catalogs.types.length
                }
              >
