...
function BackupsView({ globalQuery }) {
  ...
  async function restoreBackup(backup) {
    if (!window.confirm('Esta acción restaurará los registros y reemplazará archivos dañados. ¿Desea continuar?')) return
    setSaving(true)
    try {
      const result = await apiRequest(`/api/backups/${backup.id}/restore/`, { method: 'POST', body: { mode: 'restore' } })
      setNotice(`Restauración completada: ${result.result.database_rows_restored || 0} registros y ${result.result.files_replaced || 0} archivos reemplazados.`)
      await loadBackups()
    } catch (requestError) { setError(requestError.message) } finally { setSaving(false) }
  }
  ...
  return <div className="backups-view">
    ...
    <div className="backups-layout">
      <div className="backups-primary">
        ...
        <section className="backups-actions"><PermissionButton permission="usuarios.gestionar" className="is-primary" type="button" onClick={executeBackup} disabled={saving}><BackupIcon name="backup" size={18} /> {saving ? 'Procesando...' : 'Ejecutar respaldo'}</PermissionButton><button type="button" onClick={() => document.getElementById('backup-config')?.scrollIntoView({ behavior: 'smooth' })}><BackupIcon name="calendar" size={18} /> Programar respaldo</button><PermissionButton permission="usuarios.gestionar" type="button" onClick={() => latest && restoreBackup(latest)} disabled={!latest || saving}><BackupIcon name="restore" size={18} /> Restaurar</PermissionButton><button type="button" onClick={() => document.getElementById('backup-history')?.scrollIntoView({ behavior: 'smooth' })}><BackupIcon name="list" size={18} /> Ver historial</button></section>
        <section className="backups-panel backups-config" id="backup-config"><h2><BackupIcon name="calendar" size={18} /> Política de respaldos automáticos</h2><PermissionForm permission="usuarios.gestionar" onSubmit={saveConfiguration}><label><span>Estado</span><input type="checkbox" name="active" defaultChecked={config.active} /> Activa</label><label><span>Frecuencia</span><select name="frequency" defaultValue={config.frequency}><option value="daily">Diaria</option><option value="weekly">Semanal</option><option value="monthly">Mensual</option></select></label><label><span>Retención (días)</span><input type="number" name="retention_days" min="1" max="3650" defaultValue={config.retention_days} /></label><label><span>Destino</span><select name="destination" defaultValue={config.destination}>{data.destinations.map((destination) => <option value={destination.code} key={destination.code}>{destination.name}</option>)}</select></label><button className="is-primary" type="submit" disabled={saving}>Guardar configuración</button></PermissionForm><small>La ejecución automática utiliza el comando del programador del entorno. Las copias siempre se cifran antes de almacenarse.</small></section>
        <section className="backups-panel backups-history" id="backup-history"><h2>Historial de respaldos</h2><div className="backups-table-scroll"><table><thead><tr><th>Fecha</th><th>Tipo</th><th>Destino</th><th>Tamaño</th><th>Estado</th><th>Contenido</th><th>Acciones</th></tr></thead><tbody>{visibleBackups.map((backup) => <tr key={backup.id}><td>{formatDate(backup.started_at)}</td><td>{backupType(backup.type)}</td><td>{backup.destination}</td><td>{formatBytes(backup.size_bytes)}</td><td><span className={`backups-status backups-status--${backupStatus(backup.status).toLowerCase().replace(' ', '-')}`}>{backup.status === 'exitoso' ? <BackupIcon name="check" size={11} /> : backup.status === 'fallido' ? <BackupIcon name="failed" size={11} /> : <i />}{backupStatus(backup.status)}</span></td><td>{backup.database_records} registros · {backup.files} archivos</td><td><div><a className="backups-icon-button" aria-label={`Descargar respaldo ${backup.name}`} href={backup.download_url}><BackupIcon name="download" size={14} /></a><PermissionButton permission="usuarios.gestionar" type="button" aria-label={`Restaurar respaldo ${backup.name}`} onClick={() => restoreBackup(backup)} disabled={saving || backup.status !== 'exitoso'}><BackupIcon name="restore" size={14} /></PermissionButton></div></td></tr>)}</tbody></table>{!visibleBackups.length && <div className="backups-empty"><strong>{loading ? 'Cargando respaldos...' : 'No se encontraron respaldos'}</strong><span>Ejecute una copia manual o configure la programación automática.</span></div>}</div></section>
      </div>
       <aside className="backups-aside"><section className="backups-panel backups-side-card backups-plan"><h2><BackupIcon name="shield" size={19} /> Plan de recuperación</h2><p>Estrategia para reconstruir registros y recuperar archivos ante un incidente.</p><dl><div><dt>Plan actual</dt><dd>{data.recovery_plan.name}</dd></div><div><dt>RPO objetivo</dt><dd>{data.recovery_plan.rpo_hours} horas</dd></div><div><dt>RTO objetivo</dt><dd>{data.recovery_plan.rto_hours} horas</dd></div><div><dt>Última prueba</dt><dd>{data.recovery_plan.last_test_at ? `${formatDate(data.recovery_plan.last_test_at)} · Exitosa` : 'Pendiente'}</dd></div></dl><PermissionButton permission="usuarios.gestionar" type="button" onClick={testRecoveryPlan} disabled={!latest || saving}>Probar plan de recuperación</PermissionButton></section><section className="backups-panel backups-side-card backups-points"><h2><BackupIcon name="points" size={19} /> Puntos de restauración</h2><p>Copias cifradas disponibles para reconstruir registros y reemplazar archivos dañados.</p><dl><div><dt>Total de puntos</dt><dd>{data.restore_points.length}</dd></div><div><dt>Más antiguo</dt><dd>{data.restore_points.length ? formatDate(data.restore_points[data.restore_points.length - 1].finished_at) : '—'}</dd></div><div><dt>Más reciente</dt><dd>{latest ? formatDate(latest.finished_at) : '—'}</dd></div></dl><PermissionButton permission="usuarios.gestionar" type="button" onClick={() => latest && restoreBackup(latest)} disabled={!latest || saving}>Administrar puntos</PermissionButton></section><section className="backups-panel backups-side-card backups-alerts"><h2><BackupIcon name="bell" size={19} /> Alertas y notificaciones</h2>{data.alerts.length ? data.alerts.slice(0, 3).map((alert) => <article key={alert.id}><i className="is-red" /><div><strong>{alert.title}</strong><time>{formatDate(alert.at)}</time></div></article>) : <p>No hay fallos de respaldo registrados.</p>}</section></aside>
    </div>
  </div>
}
