...
function ReviewerReviewInboxView({ onOpenReview }) {
  ...
  const activeReviews = reviews.filter((review) => review.version?.status?.code === 'EN_REVISION')
  ...
  return <div className="reviewer-inbox-view">
    ...
    <section className="reviewer-inbox-metrics" aria-label="Resumen de revisiones">
      <article className="reviewer-inbox-metric"><span className="reviewer-inbox-metric-icon is-blue"><InboxIcon size={30} /></span><div><p>Asignados hoy</p><strong>{assignedToday.length}</strong><small>{reviews.length} {reviews.length === 1 ? 'revisión' : 'revisiones'} en total</small></div></article>
      ...
      <article className="reviewer-inbox-metric"><span className="reviewer-inbox-metric-icon is-green"><InboxIcon name="flow" size={30} /></span><div><p>Solicitudes de versiones en revisión</p><strong>{activeReviews.length}</strong><small>{completedReviews.length} revisiones resueltas</small></div></article>
    </section>
    <div className="reviewer-inbox-layout">
      <main>
        <section className="reviewer-inbox-filters" aria-label="Filtros de revisiones">
          ...
          <label><span>Estado de solicitud</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Todos los estados</option>{statuses.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
          ...
          <label className="reviewer-inbox-author"><span>Autor</span><select value={author} onChange={(event) => setAuthor(event.target.value)}><option value="">Todos los autores</option>{authors.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
