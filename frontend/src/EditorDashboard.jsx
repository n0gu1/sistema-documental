...
import { editorViews } from './permissionPolicy'
...
function EditorDashboard({ user, onLogout, logoutPending, error }) {
  ...
  const publishedDocuments = documents.filter((document) => document.publication?.published_version_ids?.length > 0)
  ...
  function dashboardView() {
    return <><header className="editor-heading"><div><h1>Dashboard del Editor</h1><p>Gestiona tus documentos, versiones y actividades personales.</p></div><time><EditorIcon name="calendar" size={18} />{today}</time></header>{notice && <p className="editor-dashboard-notice" role="status">{notice}</p>}<section className="editor-dashboard-metrics"><Metric icon="document" tone="blue" label="Mis documentos" value={documents.length} detail="Documentos registrados" /><Metric icon="history" tone="orange" label="Pendientes de revisión" value={pendingDocuments.length} detail="Borradores y revisiones" /><Metric icon="document" tone="violet" label="Documentos publicados" value={publishedDocuments.length} detail="Con alguna versión publicada" /><Metric icon="layers" tone="teal" label="Versiones activas" value={versionedDocuments.length} detail="Documentos con versión" /></section><div className="editor-dashboard-upper"><section className="editor-card editor-dashboard-recent"><div className="editor-card__heading"><div><h2><EditorIcon name="history" size={18} /> Documentos modificados recientemente</h2><p>Últimos documentos disponibles en tu espacio.</p></div><PermissionButton permission={editorViews['documents']} type="button" onClick={() => navigate('documents')}>Ver todos</PermissionButton></div><div className="editor-dashboard-activity-list">{filteredDocuments.slice(0, 5).map((document) => <article key={document.id}><span className="editor-dashboard-activity-dot" /><div><strong>{document.title}</strong><a href="#documents" onClick={(event) => { event.preventDefault(); openDocumentEditor(document) }}>{document.code} · {document.status}</a></div><time>{formatDate(document.updated_at)}</time></article>)}{!loading && !filteredDocuments.length && <p className="editor-empty">No hay documentos para mostrar.</p>}</div></section><section className="editor-card editor-dashboard-pending"><div className="editor-card__heading"><div><h2><EditorIcon name="history" size={18} /> Documentos pendientes</h2><p>Registros que requieren seguimiento.</p></div><span>{pendingDocuments.length}</span></div><div className="editor-dashboard-pending-list">{pendingDocuments.slice(0, 4).map((document) => <article key={document.id}><span className="editor-dashboard-pending-icon"><EditorIcon name="document" size={17} /></span><div><strong>{document.title}</strong><span>{document.code}</span></div><em className={`is-${statusClass(document.status)}`}>{document.status}</em></article>)}{!loading && !pendingDocuments.length && <p className="editor-empty">No hay documentos pendientes.</p>}</div><PermissionButton permission={editorViews['documents']} className="editor-dashboard-link" type="button" onClick={() => navigate('documents')}>Abrir documentos <span>→</span></PermissionButton></section><section className="editor-card editor-dashboard-status"><div className="editor-card__heading"><div><h2><EditorIcon name="chart" size={18} /> Estado de documentos</h2><p>Distribución de tus documentos.</p></div></div><div className="editor-dashboard-status-list">{statusCounts.map((item) => <div key={item.status}><span>{item.status}</span><i><b style={{ width: `${documents.length ? (item.count / documents.length) * 100 : 0}%` }} /></i><strong>{item.count}</strong></div>)}{!loading && !statusCounts.length && <p className="editor-empty">No hay estados registrados.</p>}</div></section></div><div className="editor-dashboard-lower"><section className="editor-card editor-dashboard-documents"><div className="editor-card__heading"><div><h2><EditorIcon name="document" size={18} /> Documentos recientes</h2><p>Documentos ordenados por última actualización.</p></div><PermissionButton permission={editorViews['documents']} type="button" onClick={() => navigate('documents')}>Ver todos</PermissionButton></div><div className="editor-dashboard-table-wrap"><table><thead><tr><th>Código</th><th>Título</th><th>Estado</th><th>Última actualización</th><th>Versión</th></tr></thead><tbody>{filteredDocuments.slice(0, 4).map((document) => <tr key={document.id}><td>{document.code}</td><td><PermissionButton permission="documentos.consultar" type="button" onClick={() => openDocumentEditor(document)}>{document.title}</PermissionButton></td><td><span className={`editor-dashboard-status is-${statusClass(document.status)}`}>{document.status}</span></td><td>{formatDate(document.updated_at)}</td><td>{document.version || '—'}</td></tr>)}</tbody></table>{!loading && !filteredDocuments.length && <p className="editor-empty">No hay documentos para mostrar.</p>}</div></section><section className="editor-card editor-dashboard-actions"><div className="editor-card__heading"><div><h2><EditorIcon name="plus" size={18} /> Accesos rápidos</h2><p>Ir a las áreas disponibles del editor.</p></div></div><div className="editor-dashboard-actions-grid"><PermissionButton permission={editorViews['documents']} type="button" onClick={() => navigate('documents')}><EditorIcon name="document" size={27} /><strong>Mis documentos</strong></PermissionButton><PermissionButton permission={editorViews['versions']} type="button" onClick={() => navigate('versions')}><EditorIcon name="layers" size={27} /><strong>Versiones</strong></PermissionButton><PermissionButton permission={editorViews['audit']} type="button" onClick={() => navigate('audit')}><EditorIcon name="history" size={27} /><strong>Bitácora personal</strong></PermissionButton><PermissionButton permission={editorViews['reports']} type="button" onClick={() => navigate('reports')}><EditorIcon name="chart" size={27} /><strong>Reportes básicos</strong></PermissionButton></div></section></div></>
  }
  ...
  return <main className="editor-shell editor-dashboard-shell"><aside className={`editor-sidebar${sidebarOpen ? ' is-open' : ''}`}><EditorBrand /><nav className="editor-nav" aria-label="Navegación del editor"><PermissionButton permission={editorViews['dashboard']} className={activeView === 'dashboard' ? 'is-active' : ''} type="button" onClick={() => navigate('dashboard')}><EditorIcon name="dashboard" size={22} /> Dashboard</PermissionButton><PermissionButton permission={editorViews['documents']} className={activeView === 'documents' || activeView === 'edit-document' ? 'is-active' : ''} type="button" onClick={() => navigate('documents')}><EditorIcon name="document" size={22} /> Documentos</PermissionButton><PermissionButton permission={editorViews['versions']} className={activeView === 'versions' ? 'is-active' : ''} type="button" onClick={() => navigate('versions')}><EditorIcon name="layers" size={22} /> Versiones</PermissionButton><PermissionButton permission={editorViews['audit']} className={activeView === 'audit' ? 'is-active' : ''} type="button" onClick={() => navigate('audit')}><EditorIcon name="history" size={22} /> Bitácora personal</PermissionButton><PermissionButton permission={editorViews['reports']} className={activeView === 'reports' ? 'is-active' : ''} type="button" onClick={() => navigate('reports')}><EditorIcon name="chart" size={22} /> Reportes básicos</PermissionButton></nav><div className="editor-sidebar__illustration" aria-hidden="true">♜</div><div className="editor-sidebar-footer">© {new Date().getFullYear()} Consultoría Alexandria.</div></aside>{sidebarOpen && <button className="editor-overlay" type="button" aria-label="Cerrar menú" onClick={() => setSidebarOpen(false)} />}<section className="editor-workspace"><header className="editor-topbar"><button className="editor-menu" type="button" aria-label="Abrir menú" onClick={() => setSidebarOpen(true)}><EditorIcon name="menu" size={25} /></button><label className="editor-search"><EditorIcon name="search" size={20} /><input type="search" aria-label="Buscar" placeholder="Buscar documentos, versiones, usuarios..." value={query} onChange={(event) => setQuery(event.target.value)} /></label><div className="editor-topbar__actions"><button className="editor-notification" type="button" aria-label="Notificaciones"><EditorIcon name="bell" size={23} /></button><div className="editor-profile"><button className="editor-profile__trigger" type="button" aria-expanded={profileOpen} onClick={() => setProfileOpen((open) => !open)}><span className="editor-avatar">{initials}</span><span><strong>{displayName}</strong><small>{role}</small></span><EditorIcon size={17} /></button>{profileOpen && <div className="editor-profile__menu"><span>{user.email}</span><button type="button" onClick={onLogout} disabled={logoutPending}><EditorIcon name="logout" size={16} /> {logoutPending ? 'Cerrando sesión...' : 'Cerrar sesión'}</button></div>}</div></div></header><main className="editor-content">{(error || loadError) && <p className="editor-error" role="alert">{error || loadError}</p>}<PermissionGate permission={editorViews[activeView]} fallback={<p>No tiene permiso para esta función.</p>}>{content}</PermissionGate><footer className="editor-footer"><span>© {new Date().getFullYear()} Consultoría Alexandria. Todos los derechos reservados.</span><span>Versión 2.1.0</span></footer></main></section></main>
}
