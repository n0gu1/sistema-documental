...
import { reviewerViews } from './permissionPolicy'
...
function ReviewerDashboard({ user, onLogout, logoutPending, error: initialError }) {
  ...
  return <main className="editor-shell reviewer-shell"><aside className={`editor-sidebar${sidebarOpen ? ' is-open' : ''}`}><div className="editor-brand" aria-label="Consultoría Alexandria"><div><span>Consultoría</span><strong>Alexandria</strong></div></div><nav className="editor-nav reviewer-nav" aria-label="Navegación del revisor"><PermissionButton permission={reviewerViews['dashboard']} className={activeView === 'dashboard' ? 'is-active' : ''} type="button" onClick={() => navigate('dashboard')}><ReviewerIcon size={22} />Dashboard</PermissionButton><PermissionButton permission={reviewerViews['review-inbox']} className={activeView === 'review-inbox' ? 'is-active' : ''} type="button" onClick={() => navigate('review-inbox')}><ReviewerIcon name="inbox" size={22} />Bandeja de revisión</PermissionButton><PermissionButton permission={reviewerViews['review-document']} className={activeView === 'review-document' ? 'is-active' : ''} type="button" onClick={() => navigate('review-document')}><ReviewerIcon name="document" size={22} />Documentos asignados</PermissionButton><PermissionButton permission={reviewerViews['compare']} className={activeView === 'compare' ? 'is-active' : ''} type="button" onClick={() => navigate('compare')}><ReviewerIcon name="layers" size={22} />Comparación de versiones</PermissionButton><PermissionButton permission={reviewerViews['personal-log']} className={activeView === 'personal-log' ? 'is-active' : ''} type="button" onClick={() => navigate('personal-log')}><ReviewerIcon name="history" size={22} />Bitácora personal</PermissionButton><PermissionButton permission={reviewerViews['reports']} className={activeView === 'reports' ? 'is-active' : ''} type="button" onClick={() => navigate('reports')}><ReviewerIcon name="chart" size={22} />Reportes básicos</PermissionButton></nav><div className="editor-sidebar__illustration" aria-hidden="true">♜</div></aside>{sidebarOpen && <button className="editor-overlay" type="button" aria-label="Cerrar menú" onClick={() => setSidebarOpen(false)} />}<section className="editor-workspace"><header className="editor-topbar"><button className="editor-menu" type="button" aria-label="Abrir menú" onClick={() => setSidebarOpen(true)}><ReviewerIcon name="menu" size={25} /></button><label className="editor-search"><ReviewerIcon name="search" size={20} /><input type="search" aria-label="Buscar" placeholder="Buscar documentos..." value={query} onChange={(event) => setQuery(event.target.value)} /></label><div className="editor-topbar__actions"><button className="editor-notification" type="button" aria-label="Notificaciones"><ReviewerIcon name="bell" size={22} /></button><button className="editor-profile" type="button" onClick={() => setProfileOpen((current) => !current)}><span>{initials(displayName)}</span><div><strong>{displayName}</strong><small>{role}</small></div><b>⌄</b></button>{profileOpen && <div className="editor-profile-menu"><span>{user.email}</span><button type="button" disabled={logoutPending} onClick={onLogout}>{logoutPending ? 'Cerrando...' : 'Cerrar sesión'}</button></div>}</div></header><div className="reviewer-page-content"><PermissionGate permission={reviewerViews[activeView]} fallback={<p>No tiene permiso para esta función.</p>}>{content}</PermissionGate></div><footer className="editor-footer"><span>© {new Date().getFullYear()} Consultoría Alexandria. Todos los derechos reservados.</span><span>Versión 2.1.0</span></footer></section></main>
}
