...
const emptyData = {
  summary: { total: 0, completed: 0, approved: 0, rejected: 0, returned: 0, by_status: [], by_area: [], by_type: [], by_responsible: [] },
...
function WeeklyChart({ rows, endDate }) {
  ...
  const days = Array.from({ length: 7 }, (_, index) => {
    ...
    return { date, key, count: rows.filter((row) => row.activity_date === key).length }
  })
  const max = Math.max(...days.map((day) => day.count), 1)
  const points = days.map((day, index) => `${20 + index * 60},${118 - (day.count / max) * 78}`).join(' ')
  return <div className="reviewer-report-weekly-chart"><svg viewBox="0 0 400 155" role="img" aria-label="Decisiones registradas por día durante siete días"><path className="reviewer-report-grid-line" d="M20 40H380M20 79H380M20 118H380" /><polyline points={points} /><path className="reviewer-report-area" d={`M20 118 ${points} 380 118Z`} />{days.map((day, index) => { const x = 20 + index * 60; const y = 118 - (day.count / max) * 78; return <g key={day.key}><circle cx={x} cy={y} r="4" /><text x={x} y="139" textAnchor="middle">{formatDay(day.date)}</text><text className="reviewer-report-point-value" x={x} y={Math.max(y - 10, 12)} textAnchor="middle">{day.count}</text></g> })}</svg></div>
}
...
function ReviewerBasicReportsView() {
  ...
  return <div className="reviewer-basic-reports"><header className="reviewer-reports-heading"><div><h1>Reportes básicos</h1><p>Actividad: aprobaciones, rechazos y devoluciones registrados por tu cuenta. Cada decisión cuenta una vez, en la fecha del evento.</p></div><time><ReportIcon name="calendar" size={17} />{new Intl.DateTimeFormat('es-ES', { dateStyle: 'long' }).format(new Date())}</time></header>
    ...
    <section className="reviewer-report-metrics" aria-label="Indicadores de reportes"><Metric icon="document" tone="blue" label="Decisiones registradas" value={loading ? '...' : summary.completed} detail="Aprobaciones + rechazos + devoluciones" /><Metric icon="check" tone="green" label="Aprobaciones emitidas" value={loading ? '...' : summary.approved} detail="Eventos de aprobación exitosos" /><Metric icon="comment" tone="red" label="Rechazos emitidos" value={loading ? '...' : summary.rejected} detail="Eventos de rechazo exitosos" /><Metric icon="clock" tone="violet" label="Devoluciones emitidas" value={loading ? '...' : summary.returned} detail="Eventos de devolución exitosos" /></section>
    <div className="reviewer-report-charts"><section className="reviewer-report-card reviewer-report-status-chart"><h2>Decisiones por acción</h2>{summary.by_status.length ? <StatusDonut values={summary.by_status} /> : <p className="reviewer-report-empty">No hay datos para los filtros seleccionados.</p>}</section><section className="reviewer-report-card"><h2>Decisiones por área</h2>{summary.by_area.length ? <BarChart values={summary.by_area} label="Decisiones agrupadas por área" /> : <p className="reviewer-report-empty">No hay datos para los filtros seleccionados.</p>}</section><section className="reviewer-report-card"><h2>Actividad semanal</h2><p>Siete días hasta {filters.date_to || "hoy"}. Zona: {data.activity_timezone || "zona del sistema"}. Se aplican los filtros seleccionados.</p>{rows.length ? <WeeklyChart rows={rows} endDate={filters.date_to || new Intl.DateTimeFormat("sv-SE", { timeZone: data.activity_timezone || "UTC" }).format(new Date())} /> : <p className="reviewer-report-empty">No hay datos para los filtros seleccionados.</p>}</section><section className="reviewer-report-card"><h2>Decisiones por tipo</h2>{summary.by_type.length ? <BarChart values={summary.by_type} label="Decisiones agrupadas por tipo" /> : <p className="reviewer-report-empty">No hay datos para los filtros seleccionados.</p>}</section></div>
    <section className="reviewer-report-panel reviewer-report-recent"><header><div><h2>Reportes recientes</h2><p>{history.length} reportes generados</p></div></header><div className="reviewer-report-table-scroll"><table><thead><tr><th>Nombre del reporte</th><th>Generado el</th><th>Formato</th><th>Registros</th><th>Acciones</th></tr></thead><tbody>{history.map((report) => <tr key={report.id}><td>{report.name}</td><td>{formatDate(report.created_at)}</td><td>{report.format}</td><td>{report.rows}</td><td><button type="button" onClick={() => download(report)} aria-label={`Descargar ${report.name}`}><ReportIcon name="download" size={16} />Descargar</button></td></tr>)}</tbody></table>{!history.length && <p className="reviewer-report-empty">Aún no hay reportes generados.</p>}</div><footer><div className="reviewer-report-actions"><select value={format} onChange={(event) => setFormat(event.target.value)} aria-label="Formato de reporte"><option value="PDF">PDF</option><option value="XLSX">Excel (XLSX)</option></select><PermissionButton permission="revisiones.consultar" className="is-primary" type="button" onClick={generateReport}><ReportIcon name="document" size={16} />Generar reporte</PermissionButton><button type="button" onClick={() => setShowSchedule((current) => !current)}><ReportIcon name="schedule" size={16} />Programar reporte</button></div></footer>{showSchedule && <PermissionForm permission="revisiones.consultar" className="reviewer-report-schedule" onSubmit={createSchedule}><label>Frecuencia<select value={frequency} onChange={(event) => setFrequency(event.target.value)}><option value="daily">Diaria</option><option value="weekly">Semanal</option><option value="monthly">Mensual</option></select></label><button className="is-primary" type="submit">Guardar programación</button></PermissionForm>}</section>
