...
function ReaderDocumentView({ documentId, onBack, onAction }) {
  ...
  return <div className="reader-document-view"><button className="reader-document-back" type="button" onClick={onBack}>← &nbsp;Volver a documentos disponibles</button><section className="reader-document-layout"><main><header className="reader-document-header"><span className="reader-document-file"><DocumentIcon size={34} /></span><div className="reader-document-title"><h1>{document.code} {document.title}</h1><div className="reader-document-meta"><span><b>Código</b>{document.code}</span><span><b>Área</b>{document.area?.name}</span><span><b>Tipo</b>{document.type?.name}</span><span><b>Versión</b>{version?.version || '—'}</span><span><b>Estado</b><em>{document.status?.name || 'Publicado'}</em></span><span><b>Fecha documental</b>{document.date?.split('-').reverse().join('/') || 'Sin fecha'}</span><span><b>Actualizado</b>{formatDate(document.updated_at)}</span></div></div></header><nav className="reader-document-tabs" aria-label="Secciones del documento"><button className="is-active" type="button"><DocumentIcon size={18} /> Contenido</button><PermissionButton permission="documentos.descargar" type="button" onClick={() => version?.download_url && downloadFile(version.download_url)}><DocumentIcon name="download" size={18} /> Descargar</PermissionButton><button type="button" onClick={toggleFavorite}><DocumentIcon name="star" size={18} /> {document.favorite ? 'Quitar favorito' : 'Favorito'}</button></nav><article className="reader-document-body"><h2>{document.title}</h2><p>{document.description || 'Este documento no tiene una descripción registrada.'}</p>{metadata.length > 0 && <><h3>Metadatos</h3><dl>{metadata.map(([key, value]) => <div key={key}><dt>{{ classification: 'Clasificación', observations: 'Observaciones' }[key] || key}</dt><dd>{value}</dd></div>)}</dl></>}<p className="reader-document-muted">Última actualización: {formatDate(document.updated_at)}</p></article></main><aside className="reader-document-aside"><section><h2>Acciones</h2><PermissionButton permission="documentos.descargar" type="button" onClick={() => version?.download_url && downloadFile(version.download_url)}><DocumentIcon name="download" size={17} /> Descargar versión publicada</PermissionButton><button type="button" onClick={toggleFavorite}><DocumentIcon name="star" size={17} /> {document.favorite ? 'Quitar de favoritos' : 'Agregar a favoritos'}</button></section><section><h2>Información</h2><p>Versión publicada: {version?.version || '—'}</p><p>Publicada: {formatDate(version?.published_at)}</p></section></aside></section></div>
}
