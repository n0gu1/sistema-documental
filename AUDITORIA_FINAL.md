...
**Seguimiento posterior, únicamente autorización del requisito #25 (8 de septiembre de 2026):** descarga general de archivo/versión y descarga lectora comparten comprobación explícita de `documentos.descargar` y autorización documental. Falta de permiso devuelve 403; archivo ajeno al documento devuelve 404. Veinticinco comprobaciones aprobadas contra Neon con Editor y Lector: consulta 200 sin descarga, descarga 403 sin permiso y descarga 200 con bytes verificados al tener ambos permisos; ACL denegada y versiones no publicadas también comprobadas. Véase [resultado del requisito 25](docs/REQUISITO_25_RESULTADO.md). Archivo temporal local, sin validar S3. Cambios locales pendientes de commit y despliegue; las URLs de listado quedan fuera del alcance solicitado.

**Seguimiento posterior, únicamente requisitos #26–#30 y #32 (8 de septiembre de 2026):** filtros compartidos de nombre, tipo, área, estado y fecha en rutas generales y lectoras; validación de parámetros e intervalos. `date_from/to` significa fecha documental y `updated_from/to` actualización; UI lectora incorpora fecha documental. `documentos.buscar` permanece activo en Neon y se exige en búsquedas filtradas. Cuarenta y nueve comprobaciones backend contra Neon y seis controles UI individuales aprobados, con paridad entre ambas rutas para lector. Véase [resultado de requisitos 26–30 y 32](docs/REQUISITOS_26_30_32_RESULTADO.md). Cambios locales pendientes de commit y despliegue; se conserva la matriz histórica.

**Seguimiento posterior, únicamente requisito #24 (8 de septiembre de 2026):** acción Archivar conectada a los listados de Administrador/Editor mediante `documentos.eliminar`, con motivo y confirmación. Baja lógica transaccional que conserva archivos, historial y metadatos; evento alineado con el catálogo real `DOCUMENTO_ELIMINADO`. Pruebas backend y UI aprobadas: crear, archivar, recargar y comprobar ausencia en listado normal; Neon confirma fecha, actor, motivo y bitácora. Véase [resultado del requisito 24](docs/REQUISITO_24_RESULTADO.md). Cambios locales pendientes de commit y despliegue; sin borrado físico ni cambios de versionado.

**Seguimiento posterior, únicamente requisito #23 (8 de septiembre de 2026):** edición normal mediante `documentos.modificar`, con siete campos autorizados y rechazo de atributos protegidos/desconocidos. Datos y metadatos se guardan en una transacción con bloqueo del documento; los fallos probados revierten todas las escrituras. Trece comprobaciones backend y dieciséis de regresión aprobadas; modificación y recarga UI verificadas contra backend local conectado a Neon, con persistencia confirmada por MCP. Véase [resultado del requisito 23](docs/REQUISITO_23_RESULTADO.md). Cambios locales pendientes de commit y despliegue. No se definió una política de versionado; se conserva la matriz histórica.
